...
class SelfDependencyError(ResolverError):
    """A task's dependency edge points at itself.
...
class CycleError(ResolverError):
    """The dependency edges form a cycle, so no valid wave ordering exists."""
...
class UnknownTaskError(ResolverError):
    """An edge references a task_id not present in the supplied task set."""
...
class DependencyGraph:
    ...
    def dependencies_of(self, task_id: int) -> list[TaskEdge]:
        """Edges describing what `task_id` depends on."""
        return list(self._dependencies_of[task_id])

    def waves(self) -> list[list[int]]:
        """Static topological generations, ignoring any run-time status.
        ...
        via `>>`. Cycle detection already happened in `build_graph`, so this
        never raises.
        """
    ...
    def ready(self, run_state: dict[int, TaskRunState]) -> list[int]:
        """Tasks that can run right now, given each task's current run state.
    ...
    def _edge_satisfied(edge: TaskEdge, upstream: TaskRunState) -> bool:
        ...
        if edge.dependency_type == "HAS_DATA":
            return upstream.status == "SUCCESS" and bool(upstream.target_count) and upstream.target_count > 0
...
def _check_acyclic(graph: DependencyGraph) -> None:
    """Raises CycleError with the offending cycle if one exists."""
