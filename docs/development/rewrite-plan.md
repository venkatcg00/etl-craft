...
| G5 | `feat/run-control` | One source of truth for scheduling. **Local mode: etl-craft is the orchestrator** and applies every rule, with run control beyond Airflow's: `mark` a task or a run `SUCCESS`, `FAILED` or `SKIPPED` with a required reason (a marked `SUCCESS` satisfies `HAS_DATA` only with a stated row count); `mark --new-run` records a stand-in upstream run so downstream gates pass where the upstream cannot run; `Orchestration.Dependency_gates: enforce | warn | off` per profile, each bypassed run flagged and `doctor` warning when gates are not enforced; `run --task_code --ignore-dependencies` and `--rerun [--with-downstream]` beside `--force`; `cancel` stops the run's running tasks (the supervisor watches for the request) and ends it `CANCELLED`, a new terminal status. Every intervention is recorded in `AUD_RUN_INTERVENTIONS` (what, from, to, who, when, why), shown by `history`, the catalog and alerts, and erases no attempt, log or error. **Remote mode: the orchestrator is the only source of truth.** `run --task_code` runs what it is told: no dependency, run-condition or cross-pipeline check, and a re-run after a clear simply runs. `--finalize-only` records the orchestrator's decisions (a task it never ran is `SKIPPED`, not run by the orchestrator; the run fails if a task failed). `generate-yml` puts the rules in the orchestrator: cross-pipeline dependencies as `ExternalTaskSensor`s, pipeline order in the global DAG, `max_active_runs: 1`. Rules a remote orchestrator does not support (`N` run conditions, `HAS_DATA` dependencies) fail in remote mode wherever they would otherwise be dropped (`validate`, `generate-yml`, `run --init-only`), each naming the pipeline, task and rule and saying the remote orchestrator does not support it, with the remedy (another dependency type, or local mode); `mark` and `cancel` refuse there, pointing to the orchestrator. In every mode the engine keeps what is execution, not scheduling: the run each task resolves, one `IN-PROGRESS` run per pipeline, every write, time limits, secrets, and the audit of every attempt | — | E2, G2 | |
