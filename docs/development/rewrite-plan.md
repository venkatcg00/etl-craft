...
| G3 | `feat/services-docs-publish` | `etl-craft publish-docs` serves the `generate-docs` site through an ngrok tunnel, configured in a new optional `Docs_site` section: `Authtoken` (a variable name), optional `Domain` (the team's reserved or custom domain; without it, the account's permanent free dev domain), and optional `Allowed_ips` (CIDR ranges, enforced by `publish-docs` itself from the visitor address ngrok forwards, since ngrok's own IP restriction is not honoured on every plan). There is no login: the site is reachable only by its link, is never listed or indexed (`X-Robots-Tag: noindex` and `robots.txt` disallow everything), and teams allow the site's domain through their own firewall; the docs name that domain. Records the published URL in the Engine DB and fails, naming both URLs, if a later publish gets a different one. Runs as a long-lived service; the site rebuilds on `generate-docs`. The ngrok SDK is an optional extra | — | G1 | done |
| G4 | `feat/services-catalog-lineage` | The `generate-docs` site as a searchable data catalog, in the style of Alation. **Assets:** a page for every table (each `TARGET_OBJECT`, and each source table found in SQL), with its columns, the tasks that write and read it, the business rules on it, its last run and row counts; pages for pipelines, tasks, rules and scripts. **Lineage graphs:** table-level and column-level lineage stitched across tasks and pipelines into one graph. From any table or column, a graph traces every path up to its root sources and down to its last consumers, with columns nested in their tables. Each edge is marked as a direct copy or a derived value (an expression, `CASE` or aggregate, which may have several sources). Clicking a column highlights its whole path. The graphs are laid out in Python and written into each page as SVG, so the site loads nothing from elsewhere and needs no graph library; a small script adds the filters and column tracing. Each page shows the closure of its asset, with a depth control for large graphs. **Search:** fuzzy search over pipelines, tasks, tables, columns, rules and documentation text, with filters by asset type. **Coverage:** SQL tasks parse with sqlglot, after `$$pipeline_id` substitution and reading `SOURCE_SQL_FILE`. Ingestion scripts appear at table level, reading from their external source. Without `--strict`, SQL that cannot be parsed keeps its task on the table graph with an "column lineage unavailable" badge and the parser's reason, and the build lists every such task. With `--strict`, the build fails on any. `--with-warehouse` optionally adds column types and comments from the warehouse. The `lineage` command gains `--upstream`, `--downstream` and `--column` for the same full traversal as text | `docs_generator.py`; the lineage from G1c | G1c, F1 | done |
| G4b | `feat/docs-nightly-refresh` | The catalog kept fresh: a `Docs_site` section with `Schedule` (cron) and `Output`; `generate-yml --docs` writes the `etl_craft_docs` DAG that runs `generate-docs` on it (cron or Task Scheduler without an orchestrator); the site is built beside the old one and swapped in whole; every page says when it was generated and warns once its run details are over a day old | — | G4 | done |
| G4c | `feat/catalog-navigation` | The catalog as chains: a Search tab, a DAGs tab (every pipeline with its name, `DESCRIPTION`, schedule, dependencies and last run; ingestion scripts; lineage gaps) and a Warehouse tab (database, schema, table, with writers, readers, pipelines and rules). Each pipeline page leads with its description and draws its DAG: tasks by dependency, other pipelines' tasks it waits for, dependency types, and in each task the tables it writes, reads and checks, every one a link. Breadcrumbs on every page (DAGs › pipeline › task › rule; Warehouse › database › schema › table), and every task, table, pipeline, rule and script mentioned is a link | — | G4 | done |
...
| G5a | `feat/remote-mode` | One source of truth for scheduling, **remote mode: the orchestrator is the only source of truth.** `run --task_code` runs what it is told: no dependency, run-condition or cross-pipeline check; a task run again after it succeeded is a new attempt that skips nothing (business rules check every rule again), and one run after the run ended (a cleared task) reopens it. `--init-only` checks no gate; `--finalize-only` records the orchestrator's decisions (a task it never ran is `SKIPPED`, not run by the orchestrator; a task left `IN-PROGRESS` is `FAILED`; the run fails if a task failed, is `SKIPPED` if it ran none) and moves no tracker. `generate-yml` puts the rules in the orchestrator: `__init__` first, then sensors on other pipelines (or the global DAG's order with `Global_dag`) and on other pipelines' tasks, with allowed and failed states per dependency type, and `max_active_runs: 1`. Rules a remote orchestrator does not support (`N` run conditions, `HAS_DATA` dependencies, a task whose dependencies have different types, and with `Global_dag` a pipeline whose pipeline dependencies do) fail in remote mode wherever they would otherwise be dropped (`validate`, `generate-yml`, `run --init-only`, exit status `18`), each naming the pipeline, task and rule and saying the remote orchestrator does not support it, with the remedy. The demo gains `metadata/remote_mode.sql`, and the e2e orchestrator runs its sensors. In every mode the engine keeps what is execution, not scheduling: the run each task resolves, one `IN-PROGRESS` run per pipeline, every write, time limits, secrets, and the audit of every attempt | `orchestrator.py` | E2, G2 || done |
| G5b | `feat/run-interventions` | **Local mode: etl-craft is the orchestrator**, with run control beyond Airflow's. `mark` a task or a run `SUCCESS`, `FAILED` or `SKIPPED` with a required reason (a marked `SUCCESS` satisfies `HAS_DATA` only with a stated row count); `mark --new-run` records a stand-in upstream run so downstream gates pass where the upstream cannot run; `cancel` stops the run's running tasks (the supervisor watches for the request) and ends it `CANCELLED`, a new terminal status. Every intervention is recorded in `AUD_RUN_INTERVENTIONS` (what, from, to, who, when, why), shown by `history`, the catalog and alerts, and erases no attempt, log or error. `mark` and `cancel` refuse in remote mode, pointing to the orchestrator | — | G5a || done |
| G5c | `feat/local-bypasses` | Local mode's bypasses: `Orchestration.Dependency_gates: enforce \| warn \| off` per profile, each bypassed run flagged and `doctor` warning when gates are not enforced; `run --task_code --ignore-dependencies` and `--rerun [--with-downstream]` beside `--force`, each recorded as an intervention | — | G5b || done |
