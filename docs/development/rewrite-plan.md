...
| P1 | `feat/project-layout` | The `etl-craft/` project directory holds everything a deployment needs: `craft-connector.yml`, `sql_files/`, `ingestion_scripts/`, `migrations/` and `logs/`. Discovery finds `etl-craft/craft-connector.yml`, and relative paths start from the project directory | `config.py` | C1, E1 | |
| P2 | `chore/linux-only` | Linux is the one supported platform: drop the Windows code paths (msvcrt locks, process groups), the macOS CI legs, and declare POSIX Linux in the package metadata | — | B5 | |
...
In parallel: A2 with A3; B2–B5; C2 with C3; P1 with P2 after E2; F1–F4 after P1 (F2 after F1); G1 with G2; H1–H3 with I1–I2.
