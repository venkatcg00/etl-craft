...
| G5c | `feat/local-bypasses` | Local mode's bypasses: `Orchestration.Dependency_gates: enforce \| warn \| off` per profile, each bypassed run flagged and `doctor` warning when gates are not enforced; `run --task_code --ignore-dependencies` and `--rerun [--with-downstream]` beside `--force`, each recorded as an intervention | — | G5b | |
