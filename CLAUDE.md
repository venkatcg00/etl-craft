...
Now that schema is confirmed, the natural build order (not started yet) is: the resolver (pure dependency-graph logic, no Airflow awareness) → the CLI, `run` first since almost everything else is either a read-only query or a thin wrapper around it → `generate-yml` → the connector/auth layer.
