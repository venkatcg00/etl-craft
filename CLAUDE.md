...
- **CLI entry point — done (2026-09-19), `run` only.** `src/etl_craft/cli.py`'s `main(argv)` wires up `etl-craft run --pipeline_code X --task_code Y [--force]` (matching the CLI surface table below exactly — `--pipeline_code` required, `--task_code` optional); every exception `run_task`/`load_config`/`build_engine` can raise for a non-bug reason (`CfgError`, `RunLogError`, `ConfigError`/`ConnectionError_`, `ForceNotAllowedError`, `DependenciesNotMetError`) is caught and printed as a clean one-line `error: ...` rather than a raw traceback. `src/etl_craft/__init__.py`'s `main()` (the `[project.scripts]` target) and `__main__.py` (for `python -m etl_craft`) both delegate to it. Manually verified end-to-end against Docker Postgres — happy path, unknown pipeline/task code, missing required arg, `python -m` — before writing the 4 automated tests covering the same ground in `tests/test_integration.py`.
  - `--task_code` omitted (the pipeline-level orchestration path) is refused with a clear "not implemented yet" message and exit code 2, rather than silently doing the wrong thing.
- **Pipeline-level orchestration (`run` with `--task_code` omitted), crash detection, `generate-yml`, connector/auth layer (Data DB, cloning) — not started.**
