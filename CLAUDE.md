...
- **CLI (`run`) — not started.** Next up. Needs the actual Postgres access layer (Engine DB connection, `craft-connector.yml` parsing, `AUD_PIPELINES_RUN_LOG`/`AUD_TASK_RUN_LOG` read/write) that the resolver deliberately has none of.
- **`generate-yml`, connector/auth layer — not started.**
