...
- **Engine DB access layer — done (2026-09-19).** The plumbing `run` needs, built ahead of the CLI itself since it has the most unresolved ambiguity to flag:
  ...
  - Tested against an in-memory SQLite stand-in (50 tests total across `test_config.py`/`test_db.py`/`test_runlog.py`) since no local Postgres is available in this environment — `sql/schema_test.sql` remains the authority for the real partial-unique-index race behavior against actual Postgres.
