...
\echo '=== TEST: SCHEMA_EVOLUTION defaults to false, and can be set true (expect false, then true) ==='
SELECT SCHEMA_EVOLUTION FROM CFG_TASKS WHERE TASK_CODE = 'load' AND PIPELINE_ID = 1;
INSERT INTO CFG_TASKS (TASK_CODE, TASK_TYPE, PIPELINE_ID, HANDLER, SCHEMA_EVOLUTION) VALUES ('evolvable_load', 'ETL', 1, 'SQL', TRUE) RETURNING TASK_ID, SCHEMA_EVOLUTION;
...
\echo '=== EXPECT SUCCEED: RETURN_VALUES with the two mandatory tokens plus an arbitrary extra one, pipe-separated (relaxed post-signoff — see schema.sql) ==='
INSERT INTO CFG_TASKS (TASK_CODE, TASK_TYPE, PIPELINE_ID, HANDLER, SCRIPT_NAME, RETURN_VALUES) VALUES ('good_rv', 'INGESTION', 1, 'PYTHON', 'x.py', 'INGESTION_COUNT|LATEST_OFFSET_UPDATE|SOME_CUSTOM_VAR') RETURNING TASK_ID, RETURN_VALUES;

\echo '=== EXPECT FAIL (check_violation): RETURN_VALUES with a lowercase/invalid-shaped token ==='
